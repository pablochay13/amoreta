<!DOCTYPE html>
<html>

<head>
    <meta charset="utf-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1, user-scalable=no" />
    <title>Servicios de Maquillaje para Eventos Sociales en Cancún: ¡Deslumbra en Cualquier Ocasión! - Amoreta Make Up </title>
    <meta name="description" content="En nuestra agencia de maquillaje en Cancún, nos especializamos en resaltar la belleza natural de cada persona para eventos sociales y bodas inolvidables en el paradisíaco entorno de Cancún. Con un equipo de maquilladores profesionales altamente capacitados y apasionados por su arte, estamos dedicados a brindar un servicio excepcional y personalizado que realce tu estilo único y te haga sentir radiante en tu día especial. Desde maquillaje de novias hasta servicios para eventos sociales, nos enorgullece ofrecer una amplia gama de opciones para adaptarnos a tus necesidades y preferencias. Con productos de alta calidad y técnicas de última generación, te garantizamos un resultado impecable y duradero que te hará lucir y sentirte espectacular en cualquier ocasión. ¡Confía en nosotros para hacer de tu experiencia de belleza en Cancún algo verdaderamente inolvidable!">
    <meta name="keywords" content="Maquillaje para bodas en Cancún, Maquillaje para novias en Cancún, Servicio de maquillaje para eventos en Cancún, Maquilladora profesional en Cancún, Maquillaje a domicilio en Cancún, Maquillaje de larga duración en Cancún, Maquillaje para fiestas en Cancún, Maquillaje para fotos en Cancún, Maquillaje natural en Cancún, Estudio de maquillaje en Cancún">
    <link rel="icon" href="images/icon.png" type="image/x-icon">
    <link rel="stylesheet" href="assets/css/main.css" />
    <noscript>
        <link rel="stylesheet" href="assets/css/noscript.css" />
    </noscript>

    <link rel="stylesheet" href="assets/bootstrap/css/bootstrap.min.css" />
    <!-- <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-QWTKZyjpPEjISv5WaRU9OFeRpok6YctnYmDr5pNlyT2bRjXh0JMhjY6hW+ALEwIH" crossorigin="anonymous" /> -->

</head>

<body class="is-preload borde">

    <!-- Wrapper -->
    <div id="wrapper">
        <!-- Cambio de idioma -->
        <div class="container-fluid idiom">
            <div class="row text-center">
                <div class="col-12 col-md-10"></div>

                <div class="col-12 col-md-2">
                    <div class="row">
                        <div class="col">
                            <a class="text-green" href="index.php?idioma=es">Español</a>
                        </div>
                        <div class="col">
                            <a class="text-green" href="index.php?idioma=en">Ingles</a>
                        </div>
                    </div>
                </div>
            </div>
        </div>

        <!-- Header -->
        <header id="header" class="pb-5">
            <img src="images/amoreta-header.webp" alt="" class="container px-5 img-fluid d-none d-sm-block d-md-block" />
            <img src="images/amoreta-header-mobile.webp" alt="" class="container px-5 img-fluid d-md-none" />
            <!-- <div class="logo">
				<img src="images/amoreta-logo-white.png" alt="" class="img-fluid">
			</div> -->
            <div class="content">
                <div class="inner">
                    <h1 class="text-green display-6">AMORETA AGENCY</h1>
                    <p class="px-5 text-green">Cancún | Riviera Maya | Mérida</p>
                    <!-- <img src="images/amoreta-boda.webp" alt="" class="pt-3 w-50 img-fluid"> -->
                </div>
            </div>

            <nav>
                <ul>
                    <li class="col-12 col-md-3">
                        <div class="dropdown">
                            <a class="dropbtn" style="cursor: pointer">{servicios}</a>
                            <div class="dropdown-content">
                                <a href="#bodas">{novias}</a>
                                <a href="#sociales">{sociales}</a>
                            </div>
                        </div>
                    </li>

                    <li class="col-12 col-md-3">
                        <div class="dropdown">
                            <a class="dropbtn" style="cursor: pointer">{photoshoot}</a>
                            <div class="dropdown-content">
                                <a href="#producciones">{producciones}</a>
                                <a href="#editorial">{editorial}</a>
                            </div>
                        </div>
                    </li>
                    <li class="col-12 col-md-3">
                        <a class="" href="#about">{quienesomos}</a>
                    </li>
                    <li class="col-12 col-md-3"><a href="#contact">{contacto}</a></li>
                </ul>
            </nav>
        </header>

        <!-- Main -->
        <div id="main">
            <!-- SERVICES -->
            <article id="intro">
                <h2 class="major text-white">{servicios}</h2>

                <div class="text-center pt-5 mt-5">
                    <div class="container">
                        <div class="row justify-content-md-center">
                            <div class="col-12 col-md-10 col-lg-8 col-xl-7 col-xxl-6">
                                <h3 class="fs-6 text-secondary mb-2 text-uppercase text-center text-white">
                                    {sociales}
                                </h3>
                                <hr class="w-50 mx-auto mb-5 mb-xl-9 border-dark-subtle" />
                            </div>
                        </div>
                    </div>
                    <!-- Facebook -->
                    <i class="fab fa-facebook-f text-white h4 px-3"></i>

                    <!-- Instagram -->
                    <i class="fab fa-instagram text-white h4 px-3"></i>

                    <!-- Whatsapp -->
                    <i class="fab fa-whatsapp text-white h4 px-3"></i>
                </div>
            </article>

            <!-- BODAS -->
            <article id="bodas">
                <h2 class="text-center text-green">{novias}</h2>

                <!-- Paquetes -->
                <section class="bsb-pricing-2 py-5 py-xl-8">
                    <div class="container">
                        <div class="row justify-content-md-center">
                            <div class="col-12 col-md-10 col-lg-8 col-xl-7 col-xxl-6">
                                <h3 class="text-secondary mb-2 text-uppercase text-center text-green">
                                    {nuestospaquetes}
                                </h3>
                                <hr class="w-50 mx-auto mb-5 mb-xl-9 border-dark-subtle" />
                            </div>
                        </div>
                    </div>

                    <div class="container">
                        <div class="row gy-5 gy-lg-0 gx-xl-5">
                            <div class="col-12 col-lg-4">
                                <div class="card shadow-sm">
                                    <div class="card-body p-5 p-xxl-4">
                                        <h3 class="h4 mb-2">{paqueteclasico}</h3>
                                        <ul class="list-group list-group-flush mb-4">
                                            <li class="list-group-item">
                                                <span>{clasico1}</span>
                                            </li>
                                            <li class="list-group-item">
                                                <span>{clasico2}</span>
                                            </li>
                                            <li class="list-group-item">
                                                <span>{clasico3}</span>
                                            </li>
                                        </ul>
                                        <a href="https://wa.link/4f1po7" target="_blank" class="btn bsb-btn-xl bg-butto rounded-pill">{elegir}</a>
                                    </div>
                                </div>
                            </div>

                            <div class="col-12 col-lg-4">
                                <div class="card shadow-sm">
                                    <div class="card-body p-5 p-xxl-4">
                                        <h3 class="h4 mb-2">{paquetepremium}</h3>
                                        <ul class="list-group list-group-flush mb-4">
                                            <li class="list-group-item">
                                                <span>{premium1}</span>
                                            </li>
                                            <li class="list-group-item">
                                                <span>{premium2}</span>
                                            </li>
                                            <li class="list-group-item">
                                                <span>{premium3}</span>
                                            </li>
                                            <li class="list-group-item">
                                                <span>{premium4}</span>
                                            </li>
                                        </ul>
                                        <a href="https://wa.link/4f1po7" target="_blank" class="btn bsb-btn-xl bg-butto rounded-pill">{elegir}</a>
                                    </div>
                                </div>
                            </div>

                            <div class="col-12 col-lg-4">
                                <div class="card shadow-sm">
                                    <div class="card-body p-5 p-xxl-4">
                                        <h3 class="h4 mb-2">{paquetegold}</h3>
                                        <ul class="list-group list-group-flush mb-4">
                                            <li class="list-group-item">
                                                <span>{gold1}</span>
                                            </li>
                                            <li class="list-group-item">
                                                <span>{gold2}</span>
                                            </li>
                                            <li class="list-group-item">
                                                <span>{gold3}</span>
                                            </li>
                                            <li class="list-group-item">
                                                <span>{gold4}</span>
                                            </li>
                                            <li class="list-group-item">
                                                <span>{gold5}</span>
                                            </li>
                                        </ul>
                                        <a href="https://wa.link/4f1po7" target="_blank" class="btn bsb-btn-xl bg-butto rounded-pill">{elegir}</a>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </section>

                <!-- Galeria -->
                <div class="container text-center my-5">
                    <h3 class="text-secondary mb-2 text-uppercase text-center text-green">
                        {galeria}
                    </h3>
                    <hr class="w-50 mx-auto mb-5 mb-xl-9 border-dark-subtle" />

                    <div class="row mx-auto my-auto justify-content-center">
                        <div id="recipeCarousel" class="carousel slide" data-bs-ride="carousel">
                            <div class="carousel-inner" role="listbox">
                                <div class="carousel-item active">
                                    <div class="col-md-3 p-1 border-0">
                                        <div class="card">
                                            <div class="card-img">
                                                <img src="images/bodas/1.webp" class="img-fluid" />
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="carousel-item">
                                    <div class="col-md-3 p-1">
                                        <div class="card">
                                            <div class="card-img">
                                                <img src="images/bodas/2.webp" class="img-fluid" />
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="carousel-item">
                                    <div class="col-md-3 p-1">
                                        <div class="card">
                                            <div class="card-img">
                                                <img src="images/bodas/3.webp" class="img-fluid" />
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="carousel-item">
                                    <div class="col-md-3 p-1">
                                        <div class="card">
                                            <div class="card-img">
                                                <img src="images/bodas/4.webp" class="img-fluid" />
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="carousel-item">
                                    <div class="col-md-3 p-1">
                                        <div class="card">
                                            <div class="card-img">
                                                <img src="images/bodas/5.webp" class="img-fluid" />
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="carousel-item">
                                    <div class="col-md-3 p-1">
                                        <div class="card">
                                            <div class="card-img">
                                                <img src="images/bodas/6.webp" class="img-fluid" />
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="carousel-item">
                                    <div class="col-md-3 p-1">
                                        <div class="card">
                                            <div class="card-img">
                                                <img src="images/bodas/11.webp" class="img-fluid" />
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="carousel-item">
                                    <div class="col-md-3 p-1">
                                        <div class="card">
                                            <div class="card-img">
                                                <img src="images/bodas/12.webp" class="img-fluid" />
                                            </div>
                                        </div>
                                    </div>
                                </div>

                                <div class="carousel-item">
                                    <div class="col-md-3 p-1">
                                        <div class="card">
                                            <div class="card-img">
                                                <img src="images/peinados/5.webp" class="img-fluid" />
                                            </div>
                                        </div>
                                    </div>
                                </div>

                                <div class="carousel-item">
                                    <div class="col-md-3 p-1">
                                        <div class="card">
                                            <div class="card-img">
                                                <img src="images/peinados/6.webp" class="img-fluid" />
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <!-- <a class="carousel-control-prev bg-transparent w-aut" href="#recipeCarousel" role="button"
								data-bs-slide="prev">
								<span class="carousel-control-prev-icon" aria-hidden="true"></span>
							</a>
							<a class="carousel-control-next bg-transparent w-aut" href="#recipeCarousel" role="button"
								data-bs-slide="next">
								<span class="carousel-control-next-icon" aria-hidden="true"></span>
							</a> -->
                        </div>
                    </div>
                </div>

                <!-- Carousel Testimoniales -->
                <div class="container">
                    <div class="row justify-content-md-center">
                        <div class="col-12 col-md-10 col-lg-8 col-xl-7 col-xxl-6">
                            <h3 class="text-secondary mb-2 text-uppercase text-center text-green">
                                {reviews}
                            </h3>
                            <hr class="w-50 mx-auto mb-5 mb-xl-9 border-dark-subtle" />
                        </div>
                    </div>

                    <div class="container">
                        <div class="row">
                            <div class="col-md-4 p-1 border-0 ">
                                <div class="card shadow-sm bg-card-bage">
                                    <div class="card-body">
                                        <figure>
                                            <img class="img-fluid rounded rounded-circle mb-4 border border-5" loading="lazy" src="./assets/img/testimonial-img-1.jpg" alt="" />
                                            <figcaption>
                                                <div class="bsb-ratings text-warning mb-3" data-bsb-star="5" data-bsb-star-off="0"></div>
                                                <blockquote class="bsb-blockquote-icon mb-4 reviews">
                                                    Yami, Muchas gracias por todo, el maquillaje quedó increíble, aguanto la lloradera, el baile, esa noche llegué destrozada y no me maquille y en la mañana seguía maquillada, de verdad quede encantada 1000/10 el servicio.
                                                </blockquote>
                                                <h4 class="mb-2">Idge Mejía</h4>
                                            </figcaption>
                                        </figure>
                                    </div>
                                </div>
                            </div>

                            <div class="col-md-4 p-1 border-0 ">
                                <div class="card shadow-sm bg-card-bage">
                                    <div class="card-body">
                                        <figure>
                                            <img class="img-fluid rounded rounded-circle mb-4 border border-5" loading="lazy" src="./assets/img/testimonial-img-1.jpg" alt="" />
                                            <figcaption>
                                                <div class="bsb-ratings text-warning mb-3" data-bsb-star="5" data-bsb-star-off="0"></div>
                                                <blockquote class="bsb-blockquote-icon mb-4 reviews">
                                                    Gracias por todo mi maquillaje si que quedo intacto! Lo ame muchisimo
                                                </blockquote>
                                                <h4 class="mb-2">Sofía Molina</h4>
                                            </figcaption>
                                        </figure>
                                    </div>
                                </div>
                            </div>

                            <div class="col-md-4 p-1 border-0 ">
                                <div class="card shadow-sm bg-card-bage">
                                    <div class="card-body">
                                        <figure>
                                            <img class="img-fluid rounded rounded-circle mb-4 border border-5" loading="lazy" src="./assets/img/testimonial-img-1.jpg" alt="" />
                                            <figcaption>
                                                <div class="bsb-ratings text-warning mb-3" data-bsb-star="5" data-bsb-star-off="0"></div>
                                                <blockquote class="bsb-blockquote-icon mb-4 reviews">
                                                    Betty muchas gracias por todo! No se si se dieron cuenta pero nos llovió como diez minutos después, pero todos me preguntaban que me pusieron que mi make up quedo 👌🏻! Muchas gracias.
                                                </blockquote>
                                                <h4 class="mb-2">Gaby Vargas</h4>
                                            </figcaption>
                                        </figure>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </article>

            <!-- SOCIALES -->
            <article id="sociales">
                <h2 class="text-center text-green">{sociales}</h2>

                <!-- Paquetes -->
                <section class="bsb-pricing-2 py-5 py-xl-8">
                    <div class="container">
                        <div class="row justify-content-md-center">
                            <div class="col-12 col-md-10 col-lg-8 col-xl-7 col-xxl-6">
                                <h3 class="text-secondary mb-2 text-uppercase text-center text-green">
                                    {nuestospaquetes}
                                </h3>
                                <hr class="w-50 mx-auto mb-5 mb-xl-9 border-dark-subtle" />
                            </div>
                        </div>
                    </div>

                    <div class="container">
                        <div class="row gy-5 gy-lg-0 gx-xl-5">
                            <div class="col-12 col-lg-6">
                                <div class="card shadow-sm">
                                    <div class="card-body p-5 p-xxl-4">
                                        <h3 class="h4 mb-2">{paquete1}</h3>
                                        <p>{texto1}</p>
                                        <ul class="list-group list-group-flush mb-4">
                                            <li class="list-group-item">
                                                <span>{paque1texto1}</span>
                                            </li>
                                            <li class="list-group-item">
                                                <span>{paque1texto2}</span>
                                            </li>
                                        </ul>
                                        <a href="https://wa.link/4f1po7" target="_blank" class="btn bsb-btn-xl bg-butto rounded-pill">{elegir}</a>
                                    </div>
                                </div>
                            </div>

                            <div class="col-12 col-lg-6">
                                <div class="card shadow-sm">
                                    <div class="card-body p-5 p-xxl-4">
                                        <h3 class="h4 mb-2">{paquete2}</h3>
                                        <p>{texto2}</p>
                                        <ul class="list-group list-group-flush mb-4">
                                            <li class="list-group-item">
                                                <span>
                                                    {paque2texto1}
                                                </span>
                                            </li>
                                            <li class="list-group-item">
                                                <span>
                                                    {paque2texto2}
                                                </span>
                                            </li>
                                        </ul>
                                        <a href="https://wa.link/4f1po7" target="_blank" class="btn bsb-btn-xl bg-butto rounded-pill">{elegir}</a>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </section>

                <!-- Galeria -->
                <div class="container text-center my-5">
                    <h3 class="text-secondary mb-2 text-uppercase text-center text-green">
                        {galeria}
                    </h3>
                    <hr class="w-50 mx-auto mb-5 mb-xl-9 border-dark-subtle" />

                    <div class="row mx-auto my-auto justify-content-center">
                        <div id="recipeCarousel" class="carousel slide" data-bs-ride="carousel">
                            <div class="carousel-inner" role="listbox">
                                <div class="carousel-item active">
                                    <div class="col-md-3 p-1 border-0">
                                        <div class="card">
                                            <div class="card-img">
                                                <img src="images/sociales/1.webp" class="img-fluid" />
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="carousel-item">
                                    <div class="col-md-3 p-1">
                                        <div class="card">
                                            <div class="card-img">
                                                <img src="images/sociales/2.webp" class="img-fluid" />
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="carousel-item">
                                    <div class="col-md-3 p-1">
                                        <div class="card">
                                            <div class="card-img">
                                                <img src="images/sociales/3.webp" class="img-fluid" />
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="carousel-item">
                                    <div class="col-md-3 p-1">
                                        <div class="card">
                                            <div class="card-img">
                                                <img src="images/sociales/4.webp" class="img-fluid" />
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="carousel-item">
                                    <div class="col-md-3 p-1">
                                        <div class="card">
                                            <div class="card-img">
                                                <img src="images/sociales/5.webp" class="img-fluid" />
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="carousel-item">
                                    <div class="col-md-3 p-1">
                                        <div class="card">
                                            <div class="card-img">
                                                <img src="images/sociales/6.webp" class="img-fluid" />
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="carousel-item">
                                    <div class="col-md-3 p-1">
                                        <div class="card">
                                            <div class="card-img">
                                                <img src="images/sociales/7.webp" class="img-fluid" />
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="carousel-item">
                                    <div class="col-md-3 p-1">
                                        <div class="card">
                                            <div class="card-img">
                                                <img src="images/sociales/10.webp" class="img-fluid" />
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="carousel-item">
                                    <div class="col-md-3 p-1">
                                        <div class="card">
                                            <div class="card-img">
                                                <img src="images/sociales/12.webp" class="img-fluid" />
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="carousel-item">
                                    <div class="col-md-3 p-1">
                                        <div class="card">
                                            <div class="card-img">
                                                <img src="images/sociales/13.webp" class="img-fluid" />
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="carousel-item">
                                    <div class="col-md-3 p-1">
                                        <div class="card">
                                            <div class="card-img">
                                                <img src="images/sociales/14.webp" class="img-fluid" />
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="carousel-item">
                                    <div class="col-md-3 p-1">
                                        <div class="card">
                                            <div class="card-img">
                                                <img src="images/peinados/1.webp" class="img-fluid" />
                                            </div>
                                        </div>
                                    </div>
                                </div>

                                <div class="carousel-item">
                                    <div class="col-md-3 p-1">
                                        <div class="card">
                                            <div class="card-img">
                                                <img src="images/peinados/2.webp" class="img-fluid" />
                                            </div>
                                        </div>
                                    </div>
                                </div>

                                <div class="carousel-item">
                                    <div class="col-md-3 p-1">
                                        <div class="card">
                                            <div class="card-img">
                                                <img src="images/peinados/3.webp" class="img-fluid" />
                                            </div>
                                        </div>
                                    </div>
                                </div>

                                <div class="carousel-item">
                                    <div class="col-md-3 p-1">
                                        <div class="card">
                                            <div class="card-img">
                                                <img src="images/peinados/4.webp" class="img-fluid" />
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <!-- <a class="carousel-control-prev bg-transparent w-aut" href="#recipeCarousel" role="button"
											data-bs-slide="prev">
											<span class="carousel-control-prev-icon" aria-hidden="true"></span>
										</a>
										<a class="carousel-control-next bg-transparent w-aut" href="#recipeCarousel" role="button"
											data-bs-slide="next">
											<span class="carousel-control-next-icon" aria-hidden="true"></span>
										</a> -->
                        </div>
                    </div>
                </div>

                <!-- Carousel Testimoniales -->
                <div class="container">
                    <div class="row justify-content-md-center">
                        <div class="col-12 col-md-10 col-lg-8 col-xl-7 col-xxl-6">
                            <h3 class="text-secondary mb-2 text-uppercase text-center text-green">
                                {reviews}
                            </h3>
                            <hr class="w-50 mx-auto mb-5 mb-xl-9 border-dark-subtle" />
                        </div>
                    </div>

                    <div class="container">
                        <div class="row">
                            <div class="col-md-4 p-1 border-0">
                                <div class="card shadow-sm bg-card-bage">
                                    <div class="card-body">
                                        <figure>
                                            <img class="img-fluid rounded rounded-circle mb-4 border border-5" loading="lazy" src="./assets/img/testimonial-img-1.jpg" alt="" />
                                            <figcaption>
                                                <div class="bsb-ratings text-warning mb-3" data-bsb-star="5" data-bsb-star-off="0"></div>
                                                <blockquote class="bsb-blockquote-icon mb-4">
                                                    Gracias Yam! Me gustó mucho como quedó
                                                </blockquote>
                                                <h4 class="mb-2">Paola</h4>
                                            </figcaption>
                                        </figure>
                                    </div>
                                </div>
                            </div>

                            <div class="col-md-4 p-1 border-0">
                                <div class="card shadow-sm bg-card-bage">
                                    <div class="card-body">
                                        <figure>
                                            <img class="img-fluid rounded rounded-circle mb-4 border border-5" loading="lazy" src="./assets/img/testimonial-img-1.jpg" alt="" />
                                            <figcaption>
                                                <div class="bsb-ratings text-warning mb-3" data-bsb-star="5" data-bsb-star-off="0"></div>
                                                <blockquote class="bsb-blockquote-icon mb-4">
                                                    Me encantó el resultado!
                                                    <br>
                                                    &nbsp;
                                                </blockquote>
                                                <h4 class="mb-2">Nastya</h4>
                                            </figcaption>
                                        </figure>
                                    </div>
                                </div>
                            </div>

                            <div class="col-md-4 p-1 border-0">
                                <div class="card shadow-sm bg-card-bage">
                                    <div class="card-body">
                                        <figure>
                                            <img class="img-fluid rounded rounded-circle mb-4 border border-5" loading="lazy" src="./assets/img/testimonial-img-1.jpg" alt="" />
                                            <figcaption>
                                                <div class="bsb-ratings text-warning mb-3" data-bsb-star="5" data-bsb-star-off="0"></div>
                                                <blockquote class="bsb-blockquote-icon mb-4">
                                                    Estoy feliz con mi maquillaje!
                                                    <br>
                                                    &nbsp;
                                                </blockquote>
                                                <h4 class="mb-2">Karla</h4>
                                            </figcaption>
                                        </figure>
                                    </div>
                                </div>
                            </div>

                            <div class="col-md-4 p-1 border-0 ">
                                <div class="card shadow-sm bg-card-bage">
                                    <div class="card-body">
                                        <figure>
                                            <img class="img-fluid rounded rounded-circle mb-4 border border-5" loading="lazy" src="./assets/img/testimonial-img-1.jpg" alt="" />
                                            <figcaption>
                                                <div class="bsb-ratings text-warning mb-3" data-bsb-star="5" data-bsb-star-off="0"></div>
                                                <blockquote class="bsb-blockquote-icon mb-4 reviews">
                                                    Son las mejores! Amé su profesionalismo, calidez y puntualidad, El maquillaje y peinado aguantaron perfecto el calor, el viento y el baile :)
                                                </blockquote>
                                                <!-- <h4 class="mb-2">Cristina Henandez</h4> -->
                                            </figcaption>
                                        </figure>
                                    </div>
                                </div>
                            </div>

                            <div class="col-md-4 p-1 border-0 ">
                                <div class="card shadow-sm bg-card-bage">
                                    <div class="card-body">
                                        <figure>
                                            <img class="img-fluid rounded rounded-circle mb-4 border border-5" loading="lazy" src="./assets/img/testimonial-img-1.jpg" alt="" />
                                            <figcaption>
                                                <div class="bsb-ratings text-warning mb-3" data-bsb-star="5" data-bsb-star-off="0"></div>
                                                <blockquote class="bsb-blockquote-icon mb-4 reviews">
                                                    Muchisimas gracias por el servicio chicas de @AmoretaMakeUp les quedo excelente y lo mejor su atención y disponibulidad! 1000% Recomendado!
                                                </blockquote>
                                                <!-- <h4 class="mb-2">Mariana Balado</h4> -->
                                            </figcaption>
                                        </figure>
                                    </div>
                                </div>
                            </div>

                            <div class="col-md-4 p-1 border-0 ">
                                <div class="card shadow-sm bg-card-bage">
                                    <div class="card-body">
                                        <figure>
                                            <img class="img-fluid rounded rounded-circle mb-4 border border-5" loading="lazy" src="./assets/img/testimonial-img-1.jpg" alt="" />
                                            <figcaption>
                                                <div class="bsb-ratings text-warning mb-3" data-bsb-star="5" data-bsb-star-off="0"></div>
                                                <blockquote class="bsb-blockquote-icon mb-4 reviews">
                                                    Maquillada y peinada desde las 11:00am y todo INTACTO a las 3:00am <br>
                                                    neta neta neta, solo ustedes @AmoretaMakeUp
                                                </blockquote>
                                                <!-- <h4 class="mb-2">Mariana Balado</h4> -->
                                            </figcaption>
                                        </figure>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </article>

            <!-- EDITORIAL -->
            <article id="editorial">
                <h2 class="text-center text-green">{editorial}</h2>

                <p class="text-green pt-5">
                    {textoeditorial1}
                </p>
                <p class="text-green">
                    {textoeditorial2}
                </p>

                <!-- Galeria -->
                <div class="container text-center my-5">
                    <h3 class="text-secondary mb-2 text-uppercase text-center text-green">
                        {galeria}
                    </h3>
                    <hr class="w-50 mx-auto mb-5 mb-xl-9 border-dark-subtle" />

                    <div class="row mx-auto my-auto justify-content-center">
                        <div id="recipeCarousel" class="carousel slide" data-bs-ride="carousel">
                            <div class="carousel-inner" role="listbox">
                                <div class="carousel-item active">
                                    <div class="col-md-3 p-1 border-0">
                                        <div class="card">
                                            <div class="card-img">
                                                <img src="images/editorial/1.webp" class="img-fluid" />
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="carousel-item">
                                    <div class="col-md-3 p-1">
                                        <div class="card">
                                            <div class="card-img">
                                                <img src="images/editorial/2.webp" class="img-fluid" />
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="carousel-item">
                                    <div class="col-md-3 p-1">
                                        <div class="card">
                                            <div class="card-img">
                                                <img src="images/editorial/3.webp" class="img-fluid" />
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="carousel-item">
                                    <div class="col-md-3 p-1">
                                        <div class="card">
                                            <div class="card-img">
                                                <img src="images/editorial/4.webp" class="img-fluid" />
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="carousel-item">
                                    <div class="col-md-3 p-1">
                                        <div class="card">
                                            <div class="card-img">
                                                <img src="images/editorial/5.webp" class="img-fluid" />
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="carousel-item">
                                    <div class="col-md-3 p-1">
                                        <div class="card">
                                            <div class="card-img">
                                                <img src="images/editorial/6.webp" class="img-fluid" />
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="carousel-item">
                                    <div class="col-md-3 p-1">
                                        <div class="card">
                                            <div class="card-img">
                                                <img src="images/editorial/7.webp" class="img-fluid" />
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <!-- <a class="carousel-control-prev bg-transparent w-aut" href="#recipeCarousel" role="button"
											data-bs-slide="prev">
											<span class="carousel-control-prev-icon" aria-hidden="true"></span>
										</a>
										<a class="carousel-control-next bg-transparent w-aut" href="#recipeCarousel" role="button"
											data-bs-slide="next">
											<span class="carousel-control-next-icon" aria-hidden="true"></span>
										</a> -->
                        </div>
                    </div>
                </div>
            </article>

            <!-- PRODUCCIONES -->
            <article id="producciones">
                <h2 class="text-center text-green">{producciones}</h2>

                <p class="text-green pt-5">
                    {textoproducciones}
                </p>

                <!-- Galeria -->
                <div class="container text-center my-5">
                    <h3 class="text-secondary mb-2 text-uppercase text-center text-green">
                        {galeria}
                    </h3>
                    <hr class="w-50 mx-auto mb-5 mb-xl-9 border-dark-subtle" />

                    <div class="row mx-auto my-auto justify-content-center">
                        <div id="recipeCarousel" class="carousel slide" data-bs-ride="carousel">
                            <div class="carousel-inner" role="listbox">
                                <div class="carousel-item active">
                                    <div class="col-md-3 p-1 border-0">
                                        <div class="card">
                                            <div class="card-img">
                                                <img src="images/producciones/1.webp" class="img-fluid" />
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="carousel-item">
                                    <div class="col-md-3 p-1">
                                        <div class="card">
                                            <div class="card-img">
                                                <img src="images/producciones/2.webp" class="img-fluid" />
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="carousel-item">
                                    <div class="col-md-3 p-1">
                                        <div class="card">
                                            <div class="card-img">
                                                <img src="images/producciones/3.webp" class="img-fluid" />
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="carousel-item">
                                    <div class="col-md-3 p-1">
                                        <div class="card">
                                            <div class="card-img">
                                                <img src="images/producciones/4.webp" class="img-fluid" />
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <!-- <a class="carousel-control-prev bg-transparent w-aut" href="#recipeCarousel" role="button"
											data-bs-slide="prev">
											<span class="carousel-control-prev-icon" aria-hidden="true"></span>
										</a>
										<a class="carousel-control-next bg-transparent w-aut" href="#recipeCarousel" role="button"
											data-bs-slide="next">
											<span class="carousel-control-next-icon" aria-hidden="true"></span>
										</a> -->
                        </div>
                    </div>
                </div>
            </article>

            <!-- GALLERY -->
            <article id="work">
                <h2 class="major text-white">Work</h2>

                <div class="lightbox" data-mdb-lightbox-init>
                    <div class="row">
                        <div class="col-lg-6">
                            <img src="images/bodas/image1.webp" data-mdb-img="images/bodas/image1.webp" alt="Table Full of Spices" class="w-100 mb-2 mb-md-4 shadow-1-strong rounded" />
                            <img src="images/bodas/image3.webp" data-mdb-img="images/bodas/image1.webp" alt="Coconut with Strawberries" class="w-100 shadow-1-strong rounded" />
                        </div>
                        <div class="col-lg-6">
                            <img src="images/bodas/imagen4.webp" data-mdb-img="images/bodas/image1.webp" alt="Table Full of Spices" class="w-100 mb-2 mb-md-4 shadow-1-strong rounded" />
                            <img src="images/bodas/image2.webp" data-mdb-img="images/bodas/image1.webp" alt="Coconut with Strawberries" class="w-100 shadow-1-strong rounded" />
                        </div>
                    </div>
                </div>
            </article>

            <!-- SOMOS -->
            <article id="about" class="text-center">
                <center>
                    <h2 class="major text-green text-center">AMORETA AGENCY</h2>
                </center>
                <p class="text-green">
                    {textosomos}
                </p>
                <span class="text-center main">
                    <img src="images/somos.jpg" class="img-fluid" alt="" />
                </span>
            </article>

            <!-- CONTACTO -->
            <article id="contact">
                <h2 class="major text-green">{contacto}</h2>
                <p class="text-green">{contactosub}</p>

                <form id="contact-form" method="post" action="send.php">
                    <div class="fields">
                        <div class="field half">
                            <label for="name" class="text-green">{contactonombre}</label>
                            <input type="text" name="name" id="name" />
                        </div>
                        <div class="field half">
                            <label for="email" class="text-green">{contactoemail}</label>
                            <input type="text" name="email" id="email" />
                        </div>
                        <div class="field">
                            <label for="message" class="text-green">{contactomessage}</label>
                            <textarea name="message" id="message" rows="4"></textarea>
                        </div>
                        <div class="field">
                            <div class="g-recaptcha" data-sitekey="6Lc6G9MpAAAAAMKaVzyYfYQTz3IwyJqMi2XbaMCR"></div>
                        </div>
                    </div>
                    <ul class="actions">
                        <li>
                            <button type="submit" data-callback='onSubmit' data-action='submit' value="Enviar Mensaje" class="primary">{contactobutton}</button>
                        </li>
                        <!-- <li><input type="reset" value="Reset" /></li> -->
                    </ul>
                </form>

                <ul class="icons">
                    <li>
                        <a href="https://www.tiktok.com/@amoretamakeup" class="icon brands fa-tiktok" target="_blank"><span class="label">TikTok</span></a>
                    </li>
                    <li>
                        <a href="https://www.facebook.com/amoreta.makeup" class="icon brands fa-facebook-f" target="_blank"><span class="label">Facebook</span></a>
                    </li>
                    <li>
                        <a href="https://www.instagram.com/amoreta.makeup/" target="_blank" class="icon brands fa-instagram"><span class="label">Instagram</span></a>
                    </li>
                    <li>
                        <a href="https://wa.link/4f1po7" class="icon brands fa-whatsapp" target="_blank"><span class="label">Whatsapp</span></a>
                    </li>
                    <a style="text-decoration: none; color: #616858" href="">Email: contact@amoretamakeup.com</a>
                </ul>
            </article>
        </div>

        <!-- Footer -->
        <!-- <footer id="footer">
			<p class="text-white ">&copy; Amoreta Make Up. Design: <a class="text-white"
					href="https://longinusdev.com/">Longinus Dev</a>.</p>
		</footer> -->
    </div>

    <!-- BG -->
    <div id="bg"></div>

    <!-- Scripts -->
    <script src="assets/js/jquery.min.js"></script>
    <script src="assets/js/browser.min.js"></script>
    <script src="assets/js/breakpoints.min.js"></script>
    <script src="assets/js/util.js"></script>
    <script src="assets/js/main.js"></script>

    <!-- <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js" integrity="sha384-YvpcrYf0tY3lHB60NNkmXc5s9fDVZLESaAA55NDzOxhy9GkcIdslK1eN7N6jIeHz" crossorigin="anonymous"></script> -->
    <script src="assets/bootstrap/js/bootstrap.bundle.min.js"></script>

    <script>
        let items = document.querySelectorAll(".carousel .carousel-item");

        items.forEach((el) => {
            const minPerSlide = 4;
            let next = el.nextElementSibling;
            for (var i = 1; i < minPerSlide; i++) {
                if (!next) {
                    // wrap carousel by using first child
                    next = items[0];
                }
                let cloneChild = next.cloneNode(true);
                el.appendChild(cloneChild.children[0]);
                next = next.nextElementSibling;
            }
        });
    </script>

    <script src="https://www.google.com/recaptcha/api.js"></script>

    <script>
        function onSubmit(token) {
            document.getElementById("contact-form").submit();
        }
    </script>
</body>

</html>